...
const float degreeToRadian = 3.141592653589793 / 180.0f;
enum ShotType
{
    UNUSED = 0,
    PISTOL,
...
public:
    ...
    void setState(ShotType shotType, float heading)
    {
        type = shotType;
        ...
        case PISTOL:
            body->setMass(1.5f);
            body->setVelocity(cos(heading*degreeToRadian)*20.0f, 0.0f, sin(heading*degreeToRadian)*20.0f);
        ...
        case ARTILLERY:
            ...
            body->setVelocity(cos(heading*degreeToRadian) * 40.0f, 30.0f, sin(heading*degreeToRadian) * 40.0f); // 50m/s
        ...
        case FIREBALL:
            ...
            body->setVelocity(cos(heading*degreeToRadian) * 10.0f, -0.5f, sin(heading*degreeToRadian) * 10.0); // 10m/s
        ...
        case LASER:
            ...
            body->setVelocity(cos(heading*degreeToRadian) * 100.0f, 0.0f, sin(heading*degreeToRadian) * 100.0f); // 100m/s
       ...
       case GRENADELAUNCHER:
                body->setMass(12.0f);
                body->setVelocity(cos(heading*degreeToRadian) * 10.0f, 10.0f, sin(heading*degreeToRadian) * 10.0f);
                body->setAcceleration(0.0f, -8.0f, 0.0f);
                body->setDamping(0.99f, 0.8f);
		radius = 0.3f;
                break;
        ...
        case GRENADE:
                body->setMass(12.0f);
                body->setVelocity(cos(heading*degreeToRadian) * 4.0f, 4.0f, sin(heading*degreeToRadian) * 4.0f);
                body->setAcceleration(0.0f, -3.0f, 0.0f);
                body->setDamping(0.99f, 0.8f);
		radius = 0.3f;
                break;
	}
        ...
        // Set the data common to all bodies
...
class BigBallisticDemo : public RigidBodyApplication
{
     ...
     * fired.
     */
    const static unsigned ammoRounds = 2;

    /** Holds the particle data. */
    AmmoRound ammo[ammoRounds];

    cyclone::Wind windForce;
...
windForce(cyclone::Vector3(0.0f, 0.0f, 0.0f)),
...
void BigBallisticDemo::fire()
{
    ...
    windForce.updateWindSpeed(windSpeed);
    ...
    // Set the shot
    shot->setState(currentShotType, heading);
...
void BigBallisticDemo::updateObjects(cyclone::real duration)
{
    ...
    for (AmmoRound *shot = ammo; shot < ammo+ammoRounds; shot++)
    {
        ...
        {
            // Run the physics
		//std::cout << windSpeed.magnitude() << std::endl;
            windForce.updateForce(shot->body, duration); 
	    shot->body->integrate(duration);
                ...
                shot->body->getPosition().z > 200.0f)
            {
                ...
                shot->type = UNUSED;
    		float r = (rand() / (float)RAND_MAX *20) - 10;
    		windSpeed = cyclone::Vector3(r, 0.0f, 0.0f);
...
void BigBallisticDemo::display()
{
    ...
    glVertex3f(cos(heading*degreeToRadian)*10.0f, 1.5f, sin(heading*degreeToRadian)*10.0f);
    glEnd();
    
   
    glColor3f(0.75f, 0.75f, 0.75);
    glBegin(GL_LINE_LOOP);

    for (int i = 0; i < 360; ++i)
    {
	float head = i*(degreeToRadian);
	glVertex3f(cos(head)*10, 1.5f, sin(head)*10);
    }
    glEnd();
    ...
    glColor3f(0.0f, 0.0f, 0.0f);
    char ws[30] = {};
    sprintf(ws, "%f", windSpeed.x);
    renderText(10.0f, 58.0f, ws);
