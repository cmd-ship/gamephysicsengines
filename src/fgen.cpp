...
Wind::Wind(const Vector3& wind)
: wind(wind){
}

void Wind::updateForce(RigidBody* body, real duration)
{
	if (!body->hasFiniteMass()) return;
//	body->addForce(wind*body->getInverseMass());
	body->addForce(wind);
}

void Wind::updateWindSpeed(Vector3 w) 
{
    wind = w;
}
